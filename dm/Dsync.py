...
class Dsync(object):
    ...
    def __init__(
        self, env: Dict = None, cwd: str = None, test_mode: bool = False, bsub_mode: bool = False
    ) -> None:
    ...
    def stclc_puts_resp(self) -> str:
        ...
        return self.shell.run_command(f'if [info exists resp] {{ puts $resp }} else {{ puts "ERROR" }}', self.test_mode)
    ...
    def stclc_check_in(self, files: str, comment: str = "", rec: bool = False) -> bool:
        """check in files stored in a string"""
        if not comment:
            comment = input("Please provide a comment: ")
        self.stream_command(f'set resp [ci -new {"-rec" if rec else ""} -comment "{comment}" {files}]')
        return self.stclc_check_resp_error(f"check in of {files}")

    def stclc_populate(self, url: str = "", force: bool = False, args: str = "") -> bool:
        """run the populate command, print output, and return any errors"""
        self.stream_command(f'set resp [populate -rec {"-force" if force else ""} {url} {args}]')
    ...
    def stclc_module_status(self, module: str) -> str:
        ...
        return self.shell.run_command(f"showstatus -report brief -rec -objects {module}")
    ...
    def stclc_populate_workspace(self, force: bool = False) -> bool:
        ...
        self.stream_command(f'set resp [sitr pop -skiplock {"-force" if force else ""}]')
        resp = self.stclc_puts_resp()
        if resp:
    ...
    def stclc_submit_module(self, modules: List[str], comment: str) -> bool:
        """submit the specified modules"""
        errors = {}
        for mod in modules:
            self.stream_command(f'set resp [sitr submit -force -comment "{comment}" {mod}]')
            resp = self.stclc_puts_resp()
            if resp:
                errors[mod] = resp
        if errors:
    ...
    def stclc_integrate(self, nopop: bool = False) -> bool:
        """run the sitr integrate command"""
        self.stream_command(f'set resp [sitr integrate -noprompt {"-nopop" if nopop else ""}]')
        resp = self.stclc_puts_resp()
        if resp:
    ...
    def stclc_sitr_release(self, comment: str, skip_check: bool = False, on_server: bool = False) -> bool:
    ...
    def stclc_create_branch(self, url: str, version: str, comment: str) -> bool:
        self.stream_command(f'set resp [sitr mkbranch -comment {comment} {version} {url}]')
        resp = self.stclc_puts_resp()
        if resp:
    ...
    def show_hrefs(self, url: str) -> List[Dict]:
        ...
        if not hrefs:
            ...
            return []
        return self.display_hrefs(url, hrefs)

    def display_hrefs(self, name: str, hrefs: List[Dict]) -> List[Dict]:
        """Display the hrefs for the specified URL"""
        headers = ["name", "url", "selector", "relpath"]
        out = []
        table = {}
        for header in headers:
            table[header] = [href[header] for href in hrefs if 'name' in href]
        if table['name']:
            ...
            for href in hrefs:
                if 'hrefs' in href:
                    out += [self.display_hrefs(href['name'], href['hrefs'])]
        return out

    def show_module_hrefs(self, modules, export_xls=None) -> None:
        """show the hrefs of the modules, or use the top module if not specified"""
        hrefs = {}
        if not modules:
            modules = [os.environ["SYNC_DEVAREA_TOP"]]
        for mod in modules:
            hrefs[mod] = self.show_hrefs(mod)

        if export_xls is not None:
            import pdb
            import pandas as pd
            df = pd.concat([pd.DataFrame.from_records(r) for r in hrefs['RF_DIG'][1]])
            pdb.set_trace()

    # TODO - do we need this?
    def sitr_lookup(self) -> List[Dict]:
        """run the sitr lookup and return a string with the sitr modules in the workspace"""
        return parse_kv_response(self.stclc_sitr_lookup())

    def add_href(
        self, container: str, url: str, relpath: str, test_flag: bool = False
    ) -> None:
        """add the href to the specific container"""
        if not self.stclc_mod_exists(f"{url}"):
            LOGGER.warn(f"The Href {url} does not exist")
            return
        if "@" in url:
            (base_url, selector) = url.split("@")
        else:
            (base_url, selector) = (url, [])

        for href in self.get_hrefs(container):
            if base_url == href["url"]:
                if selector == href["selector"]:
                    # LOGGER.warn(f"The Href {href['url']}@{href['selector']} is already present")
                    return
                elif test_flag:
                    LOGGER.info(
                        f"Updating the Href {href['url']} selector from {href['selector']} to {selector}"
                    )
                else:
                ...
                if test_flag:
                    LOGGER.info(
                        f"Updating the Href at {href['relpath']} from {href['url']} to {base_url}"
                    )
                else:
                    self.stclc_rm_mod(container, href['name'])
        if test_flag:
            LOGGER.info(f"Adding the new Href {url}")
        else:
            self.stclc_add_mod(container, url, relpath)
    ...
    def populate_tag(
        ...
        for mod in modules:
            ...
            if self.stclc_populate(force=force, args=args):
                errors.append(mod)
        if errors:
            LOGGER.warn(f"Errors encountered when populating {' '.join(errors)} with the tag {tag}")
            return True
        return False
    ...
    def flat_release_submit(self, sitr_mods: List[Dict], tag: str, comment: str) -> Dict:
        ...
        if self.snapshot_submit_module(sitr_mods, modules_to_submit, tag, comment):
            return {}
        return mod_list
    ...
    def create_branch(self, version: str, module_tag: str, comment: str) -> bool:
        ...
        if self.stclc_mod_exists(url):
            LOGGER.warn(f"The DSync module ({url}) already esists")
            return False
        if self.stclc_create_branch(f'{os.environ["SYNC_DEVAREA_TOP"]}%0', version, comment):
            return True
    ...
    def branch_modules(self, sitr_mods: List[Dict], mod_list: List[str], version: str, comment: str) -> True:
        ...
        for branch in branches:
            ...
            branched_url = self.get_root_url(module=branch['module'], version=f"{version}_v1.1")
    ...
    def compare(self, sitr_mods: List[Dict], modules: List[str], tag: str, is_trunk: bool, is_baseline: bool) -> None:
    ...
    def check_tag(self, sitr_mods: List[Dict], modules: List[str], tag: str) -> None:
        ...
        for mod in modules:
            ...
            parsed = parse_kv_response("\n".join(resp.splitlines()[1:-1]))  # skip first/last line
            if not parsed:
    ...
    def update_module(self, modules: List[str], config: str = "") -> bool:
        ...
        for mod in modules:
            if self.stclc_update_module(mod, config):
                errors.append(mod)
        if errors:
            LOGGER.warn(f"Errors encountered when updating {' '.join(errors)} with the config {config}")
            return True
        return False

    def display_mod_files(self, files: List[Dict]) -> None:
        """display the list of modified files in a table"""
        headers = ["name", "fetchedstate"]
        table = {}
        for header in headers:
            table[header] = [file[header] for file in files]
    ...
    def snapshot_add_submodules(self, tag: str, mod: str, hrefs: List[Dict], args: str = "") -> bool:
        ...
        for href in hrefs:
            if self.stclc_tag_files(tag, f"{href['name']}%0", args=args):
                ...
                self.stclc_add_mod(f"[url vault {mod}]\;{tag}", f"{href['url']}\;{tag}", href['relpath'])
        return status
    ...
    def check_for_submit_errors( self, modules: List[str]) -> bool:
        ...
        for mod in modules:
            ...
            if files:
                ...
                errors.add(mod)
        if errors:
            LOGGER.warn(f"Errors encountered when submitting the {' '.join(errors)} modules")
            return True
        return False

    def snapshot_submit_module(
        self, sitr_mods: List[Dict], modules: List[str], tag: str, comment: str
    ) -> bool:
        ...
        errors = set()
        for mod in modules:
            print(f"Performing the snapshot submit on {mod}")
    ...
    def get_tapeout_tag(self) -> str:
        ...
        if config == "Trunk":
            config = "v100"
        return f'tapeout_{os.environ["SYNC_DEVAREA_TOP"]}_{config}'.lower()
    ...
    def setup_tapeout_ws(self, sitr_mods: List[Dict], tag: str) -> bool:
        ...
        for mod in sitr_mods:
            if sitr_mods[mod]["status"] != "Update":
                ...
                LOGGER.warn(f'The {mod} module selector is set to {sitr_mods[mod]["selector"]}')
            else:
                continue
            if self.update_module([mod], root.stem):
                errors.append(mod)
        if errors:
            LOGGER.warn(f"Errors encountered when updating the {' '.join(errors)} modules")
            return True
        return False
    ...
    def submit(
        ...
        return self.stclc_submit_module(modules, comment)
    ...
    def sitr_integrate(self, mod_list: List[Dict] = None, nopop: bool = False) -> bool:
        ...
        if mod_list:
            ...
            for module, mod in mod_list.items():
                ...
                if not any(module_tag.startswith(p) for p in allowed_prefixes):
                    LOGGER.warning(f"Ignoring not allowed tag {module_tag} for module {module_name} on selector {branch}")
                    # TODO - raise exception?
                    continue
        ...
        if errors:
            ...
            LOGGER.warn(f"Errors encountered when submitting the {' '.join(errors)} modules")
            return True
        return self.stclc_integrate(nopop)

    def sitr_release(self, comment: str, skip_check: bool = False, on_server: bool = False) -> bool:
        """perform the sitr release"""
        return self.stclc_sitr_release(comment, skip_check=skip_check, on_server=on_server)
    ...
    def showstatus(self, modules) -> int:
        ...
        for mod in modules:
            ...
            report.extend(list(filter(None, map(str.strip, resp.splitlines())))[:-1]) # skip prompt
        ...
        for line in report:
            ...
            table['module'] += [mod if mod != last_mod else ""]  # don't repeat module on each line
