...
def main() -> int:
    ...
    parser.add_argument('-a', '--auto', action='store_true', help="Automatically update the hrefs (no prompts)")
    parser.add_argument('-x', '--xls', help="Update hrefs provided in an XLS spreadsheet")
    ...
    if 'DM_WORKSPACE_NAME' in os.environ:
        LOGGER.error(f"This script cannot be run from a DMSH shell. Please run in a different terminal window")
        return 1
